...
AI は選択された記録強度に従い、議論中の候補を内部で保持し、話題の切り替わり・結論・実際の作業の区切りで必要な変更だけを書き込みます。初出ですぐ記録したり、毎回「保持しています」と報告したりしません。明示的な記録依頼は例外です。既に保存された仮説や raw spark を勝手に削除してはいけません。

画像保存はエージェントへの指示であり、添付の自動取得を保証する機能ではありません。原本を取得できる場合は保存したファイルの実在を確認し、取得できない場合は未保存と記録します。
